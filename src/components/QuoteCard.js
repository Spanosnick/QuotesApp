...
export function QuoteCard() {
    ...
    useEffect(() => {
        const storedDailyQuotes = localStorage.getItem('dailyQuotes');
        const storedDate = localStorage.getItem('quoteLastDate');
        const todayDate = getTodayDate();

        if (storedDate === todayDate) {
            setDailyQuotes(parseInt(storedDailyQuotes, 10) || 0);
        } else {
            setDailyQuotes(0);  // Reset if the date is not the same
        }
    }, []);
    ...
    return (
        <div className="quote-card">
            ...
            <div>
                ...
                <button className="quote-card-btn" onClick={toogleFavourite} hasHeart={`${hasHeart}`} >{!hasHeart ? 'Add to' : 'Remove from'} Favourites <FontAwesomeIcon icon={faHeart} />  </button>
            </div>
        </div>
    )
}
